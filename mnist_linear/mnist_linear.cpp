...
struct ConvolutionNet : torch::nn::Module {
	ConvolutionNet()
   	...
   	fc1(320, 50),
		...
		register_module("conv2_drop", conv2);
		register_module("fc1", fc1);
		register_module("fc2", fc2);
	}

	torch::Tensor forward(torch::Tensor x) {
		x = torch::relu(max_pool2d(conv1->forward(x), 2));
		x = torch::relu(max_pool2d(conv2->forward(x), 2));
		x = x.view({ -1, 320 });
